...
```bash                                                                                                                           kali@kali:~$ sudo netdiscover -r 192.168.56.0/16
...
```bash
sudo nmap -sS -A -n -oA kioptrix12 192.168.56.109
...
I had a feeling there was still more to be found out on the website running on the target's machine so I went back to that. The website has a login form but it doesn't seem to be vulnerable to SQL injection. After some more looking around I found out that the gallery pictures could be sorted in a way that was vulnerable to SQL injection:
...
FYI: SQLmap has a built-in feature that cracks password hashes (which I chose to use when prompted), that's why these password hashes are already cracked and shown in plaintext. If I had chosen a different method and ended up with just the hashes, I would've used something like `hashcat` to crack the hashes anyway.

Anyway, both of these credentials worked when I `ssh`ed into the target's machine:
...
dreg@192.168.56.109's password:
...
loneferret@192.168.56.109's password:
Linux Kioptrix3 2.6.24-24-server #1 SMP Tue Jul 7 20:21:17 UTC 2009 i686
