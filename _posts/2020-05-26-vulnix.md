...
I SSHed into the target machine as `user` and looked around. I found one potentially interesting kernel exploit, but since `gcc` wasn't available on the target I couldn't compile the exploit correctly. After that, I spent quite some time looking for configuration vulnerabilities but I couldn't find a single thing. Neither could `linuxprivchecker.py` and `unix-privesc-check`.
...
I first used `ssh-keygen` to generate the keypair. Then, I created `/.ssh/authorized_keys` in the mounted directory. I then placed my own public key into that file so that my public key was trusted by the target:
...
After the C program compiled correctly, I SSHed back to the target, doublechecked that my binary was now 32 bit, executed it and got `root` access:
