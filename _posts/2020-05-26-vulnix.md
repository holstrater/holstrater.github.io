...
```
 ...
 I then ran an nmap scan against the target's IP address `192.168.56.121` to find out more about the target. I used the `-sS`, `-A`, `-n` and `-oA` options because in my opinion these serve a good general purpose with relatively quick but useful results. I also recently started adding the `-p-` option to make sure I don't miss any crucial ports. This checks all possible ports, not just the most common ones. I saved the output in a file because I don’t like to do multiple scans unless I absolutely have to (such as when I need different `nmap` options to fit a very specific purpose).
...
I `SSH`ed into the target machine as `user` and looked around. I found one potentially interesting kernel exploit, but since `gcc` wasn't available on the target I couldn't compile the exploit correctly. After that, I spent quite some time looking for configuration vulnerabilities but I couldn't find a single thing. Neither could `linuxprivchecker.py` and `unix-privesc-check`.

I decided that with a machine with this many open ports it isn't worth dwelling on one point of entry too much. I went back to my Kali machine and looked for another way in as a different user. I had a strong feeling that the home directory of the `vulnix` user could be important, that and/or the `sudo` rights of the `vulnix` user. This was based on the fact that these were pretty much the only things I couldn't access as the `user` user from before. I spent quite some time enumerating whatever cound be found through the other open ports, but the `nfs` service is what I kept coming back to. `nfs` lets you access remote files over a local network, so maybe I could use this to access the home directory of the `vulnix` user.
...
The mounting went fine but I couldn't actually browse the mounted directory due to a lack of permissions. After some Googling it turned out that this was probably because my current `kali` user doesn't match the user that owns the directory on the target machine. To find out what exactly that user was, I `SSH`ed back into the target to check:

```bash
kali@kali:/tmp$ ssh user@192.168.56.121
user@192.168.56.121's password:
...
So I needed to match a user called `user` with a UID of `2008`. I created the user, edited `/etc/passwd` to give it the matching UID and was now able to browse to the mounted directory:
...
I first used `ssh-keygen` to generate the keypair. Then, I created `/.ssh/authorized_keys` in the mounted directory. I then placed my own public key into that file so that my public key became trusted by the target:
...
Finally, I could `ssh` to the target machine as the `vulnix` user:
...
I continued by editing `/etc/exports` so that it would give me a new, second directory to mount. I chose `/tmp` on the target machine. After saving the file, I had to restart the VM to activate the changes I made. Unfortunately this was the only time efficient way to reboot the target machine that I could think of - the alternative was crashing it.
...
Note that the target machine is 32 bit, whereas my host machine is 64 bit. This means that 64 bit exploits, binaries, executables, etc. to gain root access simply won't work and will result in a `Cannot execute binary file` error. In order to let myself execute the small program I created, I had to compile it with the `-m32` option. This then required a lot of 32 bit libraries to be present on my system.
...
After the C program compiled correctly, I `ssh`ed back to the target, doublechecked that my binary was now 32 bit, executed it and got `root` access:
