...
I use `system()` for the reverse shell because I had some issues with properly returning from `execv()` which was my first option. Using `system()` means the program will go into a blocking mode as long as the reverse shell is active, which creates the small downside of the CPU and battery monitoring being paused. However, since the reverse shell is this program's primary purpose anyway, it's not that bad. Besides, the program is lightweight so running it shouldn't significantly be increasing any of the measured values.

Once the `system()` function exits (either because no listener was active or because the listener cancelled the shell), it's important to properly clean up with regards to the socket and stdin, stdout and stderr. I close the socket, restore stdin, stdout and stderr from the backup variables I created earlier and close them.  
