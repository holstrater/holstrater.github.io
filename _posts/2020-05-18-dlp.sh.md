...
The traffic found by `tcpdump` is saved in a file once a threshold is reached, after which that file is checked for the keywords from earlier. If one or more matches are found, these keyword matches along with the file containing the traffic are added to an alert email sent to the system administrator's email address. This email also contains the identifiers unique to the user.
